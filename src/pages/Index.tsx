...
const Index = () => {
  ...
  const handleSearch = (term: string) => {
    ...
    const searchResults = cars.filter(car => {
      return car.brand.toLowerCase().includes(searchLower) || car.model.toLowerCase().includes(searchLower) || car.title.toLowerCase().includes(searchLower) || car.location.toLowerCase().includes(searchLower);
    });
    setFilteredCars(searchResults);
  };
  const handleFavorite = (id: string) => {
    const updatedCars = cars.map(car => car.id === id ? {
      ...car,
    ...
    const updatedFilteredCars = filteredCars.map(car => car.id === id ? {
      ...car,
      isFavorite: !car.isFavorite
    } : car);
        ...
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="md:w-1/4">
              <CarFilters brands={brands} models={models} locations={locations} chargeTypes={chargeTypes} onFilterChange={handleFilterChange} onSearchChange={handleSearch} />
            ...
            <div className="md:w-3/4">
              ...
              {filteredCars.length > 0 ? <div className="listing-grid">
                  {filteredCars.map(car => <CarCard key={car.id} car={car} onFavorite={handleFavorite} />)}
                </div> : <div className="bg-white rounded-lg p-8 text-center shadow-md">
